...
ВАЖНО про круглые скобки (...):
- Текст в круглых скобках — это ЛИЧНЫЕ ЗАМЕТКИ монтёра для себя: место работ,
  этаж, кабинет, ориентир, имя контакта, любые уточнения.
- Из содержимого скобок НИЧЕГО структурного НЕ извлекай: ни материалы, ни адрес,
  ни номер акта, ни дату, ни количество.
- Принимай сокращения внутри скобок буквально: «каб» = кабинет, «эт» = этаж,
  «кв» = квартира, «п» = подъезд и т. п. — это НЕ «кабель», «этикетка» и т. д.
- Сами скобки и их содержимое целиком сохрани в work_done как есть
  (например: «замена адаптера на ОРК (орк на 1 эт. 102 каб)»).
- Материалы и прочие поля бери только из текста ВНЕ скобок.
...
async def merge_ticket(user_text: str, pending: dict) -> dict:
    ...
    merge_system = (
        ...
        "- Текст в круглых скобках (...) — это личные заметки монтёра "
        "(кабинет, этаж, ориентир). Из скобок НИЧЕГО не извлекай в материалы "
        "или другие поля; сокращения внутри скобок («каб», «эт», «кв», «п») "
        "понимай буквально (кабинет, этаж, квартира, подъезд), а не как «кабель» и т. п. "
        "Сами скобки оставь в work_done как есть.\n"
